...
export class ProductsService {
  ...
  create(createProductDto: CreateProductDto) {
    return this.productStore.create(createProductDto);
  }

  getProducts(paginate: GetProductsDto): Promise<ProductPaginator> {
    return this.productStore.findPaginate(paginate);
    // if (!page) page = 1;
    // if (!limit) limit = 30;
  ...
  getProductBySlug(slug: string): Promise<Product> {
    return this.productStore.findBySlug(slug);
  }
